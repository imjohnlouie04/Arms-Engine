...
VIEWER_SCRIPT_TEMPLATE = """import json
import os
import re
import shutil
import sys
...
def summarize_workspace(project_root):
    ...
    brand_ready = bool(brand_content.strip()) and not any(token in brand_content for token in PLACEHOLDER_BRAND_TOKENS)
...
class InitActivityMonitor:
    ...
    def _write_viewer_script(self):
        with open(self.viewer_script_path, "w", encoding="utf-8") as f:
            f.write(VIEWER_SCRIPT_TEMPLATE)
