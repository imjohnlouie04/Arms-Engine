...
""",
    ...
    def test_init_syncs_memory_approval_gate_into_gemini_agent_and_engine_instructions(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("Every net-new request that creates durable work must be logged to `.arms/SESSION.md`", gemini_agent)
            self.assertIn("This applies to all plain chat messages", gemini_agent)
            self.assertIn("Route to the correct specialist agent", gemini_agent)
            self.assertIn("arms task log --task", gemini_agent)
            self.assertIn("Before appending to or editing `.arms/MEMORY.md`, ask the user explicitly.", engine_instructions)
            self.assertIn("⚠️ Prompt Intake Record — MANDATORY GATE:", engine_instructions)
            self.assertIn("net-new feature request, bug fix, audit, code review, improvement, refactor", engine_instructions)
            self.assertIn("run `arms task log --task", engine_instructions)
