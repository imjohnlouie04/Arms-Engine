...
- **Prompt Intake Ledger:** Only prompts that start or materially change durable work should create or update a row in `.arms/SESSION.md`. First check whether the message belongs to an existing custom prompt / generated specialist prompt or an already-open task. Clarifying questions, approvals, status nudges, and issue-specific follow-ups should stay attached to that current row instead of spawning a new one. Route true net-new asks to the proper specialist agent; reserve `arms-main-agent` for orchestration, planning, session/memory maintenance, and protocol/meta work.
