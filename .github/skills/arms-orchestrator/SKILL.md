...
- **Delegation Mandate:** Assigning a row does not switch the host tool into the specialist. After assignment, hand the implementation turn to the assigned agent — Claude Code: run the assigned agent as a subagent via the Task tool; Codex CLI: spawn the assigned custom agent (from `.codex/agents/*.toml`) and wait for its result; Copilot CLI: `/agent <assigned-agent>`; other CLIs: switch to the matching agent mirror — using the model tier in the row's `Model` column. The orchestrator plans, routes, and verifies; it must not implement specialist-assigned tasks inline.
